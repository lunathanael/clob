...
TEST_CASE("order_book_multiple_orders") {
  ...
  for (LimitOrder::id_t id = 1; id <= 6; ++id) {
    CHECK(order_book.pending_orders.find(id) != order_book.pending_orders.end());
  }
}
...
TEST_CASE("order_book_edge_cases") {
  ...
  auto max_order = std::make_unique<LimitOrder>(UINT_FAST64_MAX, UINT_FAST64_MAX,
                                               UINT_FAST32_MAX, UINT_FAST32_MAX);
  order_book.add_ask_order(max_order.get());
  CHECK(order_book.pending_orders.size() == 2);
  CHECK(order_book.pending_orders.find(UINT_FAST64_MAX) != order_book.pending_orders.end());
